...
export const getMockEmissionManager = async (address?: tEthereumAddress) =>
  await MockEmissionManagerFactory.connect(
    address ||
      (
        await getDb().get(`${eContractid.MockEmissionManager}.${DRE.network.name}`).value()
      ).address,
    await getFirstSigner()
  );
...
export const getRnbwIncentivesController = async (address?: tEthereumAddress) =>
  await RnbwIncentivesControllerFactory.connect(
    address ||
      (
        await getDb().get(`${eContractid.RnbwIncentivesController}.${DRE.network.name}`).value()
      ).address,
    await getFirstSigner()
  );
...
export const getCurveFactoryMock = async (address?: tEthereumAddress) =>
  await CurveFactoryMockFactory.connect(
    address ||
      (
        await getDb().get(`${eContractid.CurveFactoryMock}.${DRE.network.name}`).value()
      ).address,
    await getFirstSigner()
  );
